# SPRINT-01-00 — Bootstrap do aplicativo web

**Fase:** 01 — Fundação Web
**Status:** COMPLETE_WITH_INTEGRATION_REQUIREMENTS
**Dependências:** nenhuma
**Superfície principal:** a aplicação executável, os scripts mínimos e a primeira rota do canvas full-screen

## Objetivo

Entregar a aplicação executável, os scripts mínimos e a primeira rota do canvas full-screen como uma fatia utilizável do workspace. A fundação deve deixar claro que o canvas ocupa toda a viewport e que o produto não começa como um quadro Kanban.

## Trabalho

1. Definir o fluxo principal de a aplicação executável, os scripts mínimos e a primeira rota do workspace e os estados que o usuário precisa compreender.
...
6. Garantir uma rota inicial de canvas em tela inteira, sem sidebar ou coluna de menu persistente.
...
- Canvas inicial em grade espacial única, preparado para tiles de agentes e overlays.
...
- O canvas ocupa a viewport; menus ficam em sheets/drawers e controles simples são icon-only sobrepostos.
