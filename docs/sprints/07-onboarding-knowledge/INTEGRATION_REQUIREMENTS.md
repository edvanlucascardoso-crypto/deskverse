...
- áudio → Vercel AI Gateway (`openai/whisper-1`) → `InferenceGateway` com `gpt-5.6-luna`, raciocínio solicitado `medium`, sem `max`; sem `AI_GATEWAY_API_KEY` e sem OIDC, a versão fica em `WAITING_USER`;
...
5. Configurar `AI_GATEWAY_API_KEY` ou habilitar OIDC da Vercel, testar Whisper pelo Gateway com arquivo autorizado e validar a revisão `gpt-5.6-luna` `medium` em ambiente controlado. Os testes locais usam providers fake e não fazem chamada externa.
