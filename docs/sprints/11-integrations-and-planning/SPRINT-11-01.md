# SPRINT-11-01 — Criação visual assistida

**Fase:** 11 — Integrações e Planejamento
**Status inicial:** PLANNED
**Dependências:** dependências da fase anterior
**Superfície principal:** briefing visual, ferramenta autorizada e revisão

## Objetivo

Entregar briefing visual, ferramenta autorizada e revisão como uma fatia de produto demonstrável. A sprint deve resolver o problema descrito sem antecipar implementação de sprints posteriores e sem esconder decisões de integração.

## Trabalho

1. Descrever e implementar o caminho principal de briefing visual, ferramenta autorizada e revisão.
...
## Tool de design vetorial para agentes

Preparar a integração conceitual de um MCP/tool da aplicação clone do Figma para que agentes criem designs vetoriais. A tool não terá parte visual exposta ao usuário nesta sprint; o clone será estudado somente no momento de implementar o agente que precisar dele.

## Arquivos produzidos

Quando o agente gerar um design vetorial, composição, preview, export ou outro artefato persistente, deverá enviá-lo ao UploadThing por tool, vinculá-lo ao projeto, tarefa, versão, origem e agente responsável e disponibilizá-lo na tela de arquivos. O usuário apenas consulta o resultado na Data Table paginada com busca ou na lista de cards mobile; não há upload, edição ou exclusão manual.
