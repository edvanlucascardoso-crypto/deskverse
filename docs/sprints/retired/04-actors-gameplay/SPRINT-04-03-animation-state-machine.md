...
## OBJECTIVE
Implementar state machine de idle/walk/sit/talk/work_keyboard.
...
## REQUIREMENTS
- Adaptar AnimationMixer, Behaviors e transições nativas do Stem; não implementar state machine concorrente.
- Transitions determinísticas dirigidas por eventos semânticos do backend.
- State visual separado do backend agent status via mapper.
- Sem animation spaghetti.
- AnimationMixer lifecycle limpo dentro de Behavior/engine adapter; React não controla mixer diretamente.
- Fallback próprio só é permitido após lacuna nativa reproduzida por teste e registrada em IR/decisão.

## ALLOWED CHANGES
- packages/deskverse-engine/src/deskverse/actors/animation/**
...
- Hard prerequisites (all must be COMPLETE or accepted by the Supervisor): `SPRINT-02-06`, `SPRINT-04-00`, `SPRINT-04-02`, `SPRINT-04-11`.
...
- Registre a lacuna em `integration-requirements/SPRINT-04-03-<slug>.json`.
- Se um contrato compartilhado parecer inadequado, gere um Contract Change Request; não altere unilateralmente.

## REQUIRED OUTPUTS
- Stem animation/Behavior adapter
- semantic state map
- tests

## ACCEPTANCE / TESTS
- all 5 clips transition
- no duplicated mixer loops
- unmount cleanup
- evidência de execução no mixer/Behavior nativo e ausência de state machine paralela
