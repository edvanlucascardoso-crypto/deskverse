...
## OBJECTIVE
Adaptar a navegação nativa do Stem para agents autônomos, mapeando footprints e metadata das salas prebuilt para NavMesh/waypoints sem implementar A* paralelo.
...
## REQUIREMENTS
- Usar NavMesh, waypoints, obstacle/footprint e comandos de navegação nativos do Stem por adapter Deskverse.
- Traduzir metadata do room manifest para a representação nativa sem duplicar fonte de verdade.
- Validar bloqueio por furniture footprint e circulação dos agentes.
- Grid/A* Deskverse só pode existir como fallback após lacuna nativa reproduzida por teste e registrada em IR/decisão.

## ALLOWED CHANGES
- packages/office-navigation/**
...
- Hard prerequisites (all must be COMPLETE or accepted by the Supervisor): `SPRINT-01-05`, `SPRINT-03-01`, `SPRINT-04-00`, `SPRINT-04-11`.
...
- Registre a lacuna em `integration-requirements/SPRINT-04-04-<slug>.json`.
- Se um contrato compartilhado parecer inadequado, gere um Contract Change Request; não altere unilateralmente.

## REQUIRED OUTPUTS
- navigation/NavMesh adapter
- occupancy/waypoint mapping
- tests

## ACCEPTANCE / TESTS
- blocked corridor
- multi-footprint furniture
- no path
- rota/movimento determinístico nas mesmas entradas
- prova de uso do runtime nativo; fallback, se necessário, acompanhado de teste da lacuna e IR/decisão
