...
## OBJECTIVE
Fazer actor navegar até workstation, sentar e trabalhar via state machine.
...
## REQUIREMENTS
- Sequence: reserve -> navigate -> sit -> work.
- Cancel/release reservation.
- Não ligar a agente real ainda.
- Implementar como Stem Behavior (ou composição de Behaviors) acionável por comando semântico do Office Bridge.
- Prototipar visualmente pela bridge e entregar Behavior/Lambda Deskverse-owned de produção executado pelo Stem; não criar runtime paralelo nem traduzir protótipo automaticamente.

## ALLOWED CHANGES
- packages/deskverse-engine/src/deskverse/behaviors/workstation/**
...
- Hard prerequisites (all must be COMPLETE or accepted by the Supervisor): `SPRINT-04-05`, `SPRINT-04-06`, `SPRINT-04-11`.
...
- Registre a lacuna em `integration-requirements/SPRINT-04-07-<slug>.json`.
...
## ACCEPTANCE / TESTS
- two actors cannot reserve same slot
- cancel releases
- animation sequence
- Behavior/Lambda Deskverse-owned executa no runtime Stem e corresponde ao protótipo aprovado
